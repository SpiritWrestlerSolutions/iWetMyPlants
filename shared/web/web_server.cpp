...
const char* getGreenhouseSettingsHtml() {
    return R"rawliteral(<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0"><title>Settings - iWetMyPlants</title><link rel="stylesheet" href="/style.css"><style>.rtbl{width:100%;border-collapse:collapse;font-size:.82rem}.rtbl th{text-align:left;padding:.3rem .4rem;font-size:.75rem;color:#666;border-bottom:2px solid #e0e0e0}.rtbl td{padding:.35rem .4rem;border-bottom:1px solid #f0f0f0;vertical-align:middle}.rtbl input[type=text]{width:100%;max-width:130px;padding:.25rem .4rem;border:1px solid #ddd;border-radius:4px;font-size:.82rem}.rtbl input[type=number]{width:72px;padding:.25rem .4rem;border:1px solid #ddd;border-radius:4px;font-size:.82rem}.rtbl input[type=checkbox]{cursor:pointer;width:16px;height:16px}.row-dis td{opacity:.45}.save-row{margin-top:.8rem;display:flex;align-items:center;gap:.8rem}.msg{font-size:.82rem;display:none}.msg.ok{color:#2e7d32}.msg.err{color:#c62828}.tbl-wrap{overflow-x:auto}.hint{font-size:.78rem;color:#666;margin:.3rem 0 .6rem;padding:.4rem .6rem;background:#e8f5e9;border-radius:4px;border-left:3px solid #4caf50}</style></head><body><header><a href="/" class="back-link">&larr; Back</a><h1>Settings</h1></header><nav><a href="/">Dashboard</a><a href="/relays">Relays</a><a href="/settings" class="active">Settings</a></nav><main><div class="card"><h2>Relay Configuration</h2><div class="tbl-wrap"><table class="rtbl"><thead><tr><th>#</th><th>Name</th><th>Enabled</th><th>GPIO</th><th>Active Low</th><th>Max On (s)</th><th>Min Off (s)</th><th>Cooldown (s)</th></tr></thead><tbody id="relay-tbody"><tr><td colspan="8" style="color:#888;padding:.6rem">Loading relay config...</td></tr></tbody></table></div><div class="save-row"><button class="btn-primary" onclick="saveRelays()">Save Relay Config</button><span class="msg" id="relay-msg"></span></div></div><div class="card"><h2>Environmental Sensor</h2><div class="form-group"><label for="env-type">Sensor Type</label><select id="env-type" onchange="updateEnvFields()"><option value="0">None</option><option value="1">DHT11</option><option value="2">DHT22</option><option value="3">SHT30</option><option value="4">SHT31</option><option value="5">SHT40</option><option value="6">SHT41</option></select></div><div id="env-dht-fields" style="display:none"><div class="form-group"><label for="env-pin">Data Pin (GPIO)</label><input type="number" id="env-pin" min="0" max="39" value="4"></div></div><div id="env-i2c-fields" style="display:none"><div class="form-group"><label for="env-addr">I2C Address</label><input type="text" id="env-addr" maxlength="6" style="width:90px" placeholder="0x44"></div><p class="hint">I2C bus pins &mdash; SDA: GPIO 21 &bull; SCL: GPIO 22</p></div><div id="env-interval-row" style="display:none"><div class="form-group"><label for="env-interval">Read Interval (seconds)</label><input type="number" id="env-interval" min="5" max="3600" style="width:90px" value="60"></div></div><div class="save-row"><button class="btn-primary" onclick="saveEnvSensor()">Save Sensor Config</button><span class="msg" id="env-msg"></span></div></div><div class="card"><h2>Network</h2><div class="system-actions"><a href="/settings/wifi" class="btn btn-secondary">WiFi Settings</a><a href="/settings/mqtt" class="btn btn-secondary">MQTT Settings</a></div></div><div class="card"><h2>System</h2><div class="system-actions"><button class="btn btn-warning" onclick="if(confirm('Reboot device?')){fetch('/api/system/reboot',{method:'POST'}).then(function(){alert('Rebooting...')})}">Reboot Device</button><button class="btn btn-danger" onclick="if(confirm('Factory reset? This erases ALL settings!')){fetch('/api/config/reset',{method:'POST'}).then(function(){alert('Reset done. Rebooting...')})}">Factory Reset</button></div></div></main><script>function showMsg(id,ok,txt){var m=document.getElementById(id);m.textContent=txt;m.className='msg '+(ok?'ok':'err');m.style.display='';setTimeout(function(){m.style.display='none'},3500)}function updateEnvFields(){var t=parseInt(document.getElementById('env-type').value);var isDHT=t===1||t===2;var isSHT=t>=3;document.getElementById('env-dht-fields').style.display=isDHT?'':'none';document.getElementById('env-i2c-fields').style.display=isSHT?'':'none';document.getElementById('env-interval-row').style.display=t>0?'':'none';}function esc(s){var d=document.createElement('div');d.textContent=s;return d.innerHTML}function g(id){return document.getElementById(id)}function loadRelays(){fetch('/api/config/relays').then(function(r){return r.json()}).then(function(relays){if(!Array.isArray(relays)){throw new Error('not array');}var tb=g('relay-tbody');if(relays.length===0){tb.innerHTML='<tr><td colspan="8" style="color:#888">No relay slots found</td></tr>';return;}tb.innerHTML=relays.map(function(r,i){var en=r.enabled?'checked':'';var al=r.active_low?'checked':'';return '<tr id="rr'+i+'">'+'<td>'+(i+1)+'</td>'+'<td><input type="text" id="rn'+i+'" value="'+esc(r.name||'')+'" maxlength="31"></td>'+'<td><input type="checkbox" id="re'+i+'" '+en+' onchange="rowDim('+i+')"></td>'+'<td><input type="number" id="rp'+i+'" value="'+r.pin+'" min="0" max="39"></td>'+'<td><input type="checkbox" id="ra'+i+'" '+al+'></td>'+'<td><input type="number" id="rx'+i+'" value="'+r.max_on_time_sec+'" min="0" max="86400"></td>'+'<td><input type="number" id="rf'+i+'" value="'+r.min_off_time_sec+'" min="0" max="86400"></td>'+'<td><input type="number" id="rc'+i+'" value="'+r.cooldown_sec+'" min="0" max="86400"></td>'+'</tr>';}).join('');relays.forEach(function(r,i){rowDim(i);});}).catch(function(e){g('relay-tbody').innerHTML='<tr><td colspan="8" style="color:#c62828">Error loading relay config: '+e.message+'</td></tr>';});}function rowDim(i){var row=g('rr'+i);if(row){row.style.opacity=g('re'+i)&&g('re'+i).checked?'1':'.45';}}function saveRelays(){var tbody=g('relay-tbody');var rows=tbody.querySelectorAll('tr[id^="rr"]');if(rows.length===0){showMsg('relay-msg',false,'Table not loaded');return;}var arr=[];rows.forEach(function(row,i){arr.push({name:g('rn'+i)?g('rn'+i).value:'',enabled:g('re'+i)?g('re'+i).checked:false,pin:g('rp'+i)?parseInt(g('rp'+i).value)||0:0,active_low:g('ra'+i)?g('ra'+i).checked:false,max_on_time_sec:g('rx'+i)?parseInt(g('rx'+i).value)||0:300,min_off_time_sec:g('rf'+i)?parseInt(g('rf'+i).value)||0:60,cooldown_sec:g('rc'+i)?parseInt(g('rc'+i).value)||0:300});});fetch('/api/config/relays',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(arr)}).then(function(r){return r.json()}).then(function(d){showMsg('relay-msg',d.success!==false,'Saved! Reboot to apply.')}).catch(function(){showMsg('relay-msg',false,'Save failed');});}function loadEnvSensor(){fetch('/api/config/env_sensor').then(function(r){return r.json()}).then(function(d){g('env-type').value=d.sensor_type||0;g('env-pin').value=d.pin||4;g('env-addr').value='0x'+(d.i2c_address||68).toString(16).toUpperCase();g('env-interval').value=d.read_interval_sec||60;updateEnvFields();}).catch(function(){});}function saveEnvSensor(){var t=parseInt(g('env-type').value);var addrVal=g('env-addr').value.trim();var addr=addrVal.indexOf('x')>0?parseInt(addrVal,16):parseInt(addrVal);if(isNaN(addr))addr=68;fetch('/api/config/env_sensor',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({sensor_type:t,enabled:t!==0,pin:parseInt(g('env-pin').value)||4,i2c_address:addr,read_interval_sec:parseInt(g('env-interval').value)||60})}).then(function(r){return r.json()}).then(function(d){showMsg('env-msg',d.success!==false,'Saved!')}).catch(function(){showMsg('env-msg',false,'Save failed');});}updateEnvFields();loadRelays();loadEnvSensor();</script></body></html>)rawliteral";
}
