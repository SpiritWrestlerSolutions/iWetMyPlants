...
const char* getSecuritySettingsHtml();
...
bool WebServer::begin(const DeviceIdentity& identity) {
    ...
    // CORS — allow the external installer page to reach the device API
    // when served over HTTP. (HTTPS→HTTP is blocked by mixed-content policy
    // regardless, so the device Settings page is the upgrade path there.)
    DefaultHeaders::Instance().addHeader("Access-Control-Allow-Origin",  "*");
    DefaultHeaders::Instance().addHeader("Access-Control-Allow-Methods", "GET, POST, OPTIONS");
    DefaultHeaders::Instance().addHeader("Access-Control-Allow-Headers", "Content-Type");

    // Handle CORS preflight for the two endpoints the installer uses
    _server->on("/api/system/info", HTTP_OPTIONS, [](AsyncWebServerRequest* req) {
        req->send(204);
    });
    _server->on("/api/system/ota", HTTP_OPTIONS, [](AsyncWebServerRequest* req) {
        req->send(204);
    });
...
void WebServer::registerStaticRoutes() {
    ...
    _server->on("/settings/security", HTTP_GET, [](AsyncWebServerRequest* request) {
        LOG_D(TAG, "GET /settings/security");
        sendHtmlNoCache(request, getSecuritySettingsHtml());
    });
...
<script>
function esc(s){var d=document.createElement('div');d.textContent=s;return d.innerHTML}function ago(s){if(s==null)return '';if(s<60)return s+'s ago';if(s<3600)return Math.round(s/60)+'m ago';return Math.round(s/3600)+'h ago'}/* visibility throttle: pause polling when tab hidden */(function(){var _si=window.setInterval;window.setInterval=function(f,m){return _si(function(){if(!document.hidden)f.apply(null,arguments)},m)};})();
...
const char* getWifiSettingsHtml() {
    return R"rawliteral(<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0"><title>WiFi Settings</title><link rel="stylesheet" href="/style.css"></head><body><header><a href="/settings" class="back-link">&larr; Back</a><h1>WiFi Settings</h1></header><main><div class="card"><h2>Current Status</h2><div id="wifi-status"><div class="reading"><span class="label">Connected</span><span class="value" id="wifi-connected">--</span></div><div class="reading"><span class="label">SSID</span><span class="value" id="current-ssid">--</span></div><div class="reading"><span class="label">IP Address</span><span class="value" id="current-ip">--</span></div><div class="reading"><span class="label">Signal</span><span class="value" id="current-rssi">--</span></div></div></div><div class="card"><h2>WiFi Configuration</h2><form id="wifi-form"><div class="form-group"><label for="ssid">Network Name (SSID)</label><input type="text" id="ssid" name="ssid" maxlength="32" required></div><div class="form-group"><button type="button" id="scan-btn" class="btn btn-secondary" style="width:100%">Scan for Networks</button><div id="network-list" style="display:none;margin-top:10px;max-height:200px;overflow-y:auto"></div></div><div class="form-group"><label for="password">Password</label><input type="password" id="password" name="password" maxlength="64"><label style="font-size:.8em;margin-top:5px"><input type="checkbox" id="show-password"> Show password</label></div><div class="form-group"><label><input type="checkbox" id="static-ip" name="use_static_ip"> Use Static IP</label></div><div id="static-ip-fields" style="display:none"><div class="form-group"><label for="ip">IP Address</label><input type="text" id="ip" name="static_ip" placeholder="192.168.1.100"></div><div class="form-group"><label for="gateway">Gateway</label><input type="text" id="gateway" name="gateway" placeholder="192.168.1.1"></div><div class="form-group"><label for="subnet">Subnet Mask</label><input type="text" id="subnet" name="subnet" placeholder="255.255.255.0"></div></div><div class="form-group"><label for="hub-addr">Hub Address (for sensor reporting)</label><input type="text" id="hub-addr" name="hub_address" placeholder="192.168.1.50" maxlength="39"><small style="color:#666;display:block;margin-top:4px">Leave empty for standalone mode</small></div><button type="submit" class="btn-primary" style="width:100%">Save &amp; Connect</button></form></div><div class="card"><h2>Apply Changes</h2><p style="font-size:.84rem;color:#666;margin-bottom:.8rem">WiFi changes take effect after reboot. Save your credentials above, then reboot.</p><button class="btn btn-warning" style="width:100%" onclick="if(confirm('Reboot device?')){fetch('/api/system/reboot',{method:'POST'}).then(function(){alert('Rebooting...')})}">Reboot Device</button></div></main><script>async function fetchJson(u){return(await fetch(u)).json()}async function postJson(u,d){return(await fetch(u,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(d)})).json()}async function loadWifiStatus(){try{const d=await fetchJson('/api/status');if(d.wifi){document.getElementById('wifi-connected').textContent=d.wifi.connected?'Yes':'No';document.getElementById('current-ssid').textContent=d.wifi.ssid||'--';document.getElementById('current-ip').textContent=d.wifi.ip||'--';document.getElementById('current-rssi').textContent=d.wifi.rssi?d.wifi.rssi+' dBm':'--'}const c=await fetchJson('/api/config/wifi');if(c.ssid)document.getElementById('ssid').value=c.ssid;if(c.use_static_ip)document.getElementById('static-ip').checked=true;if(c.hub_address)document.getElementById('hub-addr').value=c.hub_address}catch(e){console.error(e)}}let scanInterval=null;document.getElementById('scan-btn').addEventListener('click',async()=>{const b=document.getElementById('scan-btn'),l=document.getElementById('network-list');b.textContent='Scanning...';b.disabled=true;l.style.display='block';l.innerHTML='<p>Scanning...</p>';await fetchJson('/api/wifi/networks');let a=0;scanInterval=setInterval(async()=>{const d=await fetchJson('/api/wifi/networks');if(!d.scanning||a++>10){clearInterval(scanInterval);b.textContent='Scan for Networks';b.disabled=false;if(d.networks&&d.networks.length>0){while(l.firstChild)l.removeChild(l.firstChild);d.networks.forEach(function(n){var row=document.createElement('div');row.className='ni';row.addEventListener('click',function(){document.getElementById('ssid').value=n.ssid;l.style.display='none';document.getElementById('password').focus()});var s1=document.createElement('span');s1.textContent=n.ssid;var s2=document.createElement('span');s2.className='sg';s2.textContent=n.rssi+' dBm'+(n.encryption?' 🔒':'');row.appendChild(s1);row.appendChild(s2);l.appendChild(row)})}else{l.innerHTML='<p>No networks found</p>'}}},1000)});document.getElementById('static-ip').addEventListener('change',e=>{document.getElementById('static-ip-fields').style.display=e.target.checked?'block':'none'});document.getElementById('show-password').addEventListener('change',e=>{document.getElementById('password').type=e.target.checked?'text':'password'});document.getElementById('wifi-form').addEventListener('submit',async function(e){e.preventDefault();const ssid=document.getElementById('ssid').value,password=document.getElementById('password').value;if(!ssid){alert('Please enter a network name');return}try{const r=await postJson('/api/wifi/connect',{ssid:ssid,password:password,hub_address:document.getElementById('hub-addr').value});alert(r.message||'Connecting...')}catch(err){alert('Error: '+err.message)}});loadWifiStatus()</script><style>.ni{padding:10px;border-bottom:1px solid #e0e0e0;cursor:pointer;display:flex;justify-content:space-between}.ni:hover{background:#f0f0f0}.ni span:first-child{font-weight:500}.sg{color:#666;font-size:.9em}</style></body></html>)rawliteral";
}

const char* getSecuritySettingsHtml() {
    return R"rawliteral(<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0"><title>Security Settings - iWetMyPlants</title><link rel="stylesheet" href="/style.css"><style>.sec-state{padding:.6rem .8rem;border-radius:6px;font-size:.9rem;font-weight:500;margin-bottom:1rem;display:flex;align-items:center;gap:.5rem}.sec-on{background:#c8e6c9;color:#2e7d32}.sec-off{background:#fff3cd;color:#856404}.sec-dot{width:10px;height:10px;border-radius:50%;display:inline-block}.sec-on .sec-dot{background:#2e7d32}.sec-off .sec-dot{background:#856404}.sec-help{font-size:.82rem;color:#666;line-height:1.55;margin-bottom:1rem;padding:.6rem .8rem;background:#f5f5f5;border-left:3px solid #2196F3;border-radius:4px}.warn{background:#fff3cd;border-left:3px solid #FF9800;padding:.6rem .8rem;border-radius:4px;font-size:.82rem;color:#856404;margin:.6rem 0}#current-row{display:none}.danger-row{margin-top:1.2rem;padding-top:1rem;border-top:1px solid #eee}</style></head><body><header><a href="/settings" class="back-link">&larr; Back</a><h1>Security</h1></header><nav><a href="/">Dashboard</a><a href="/sensors">Sensors</a><a href="/devices">Devices</a><a href="/settings" class="active">Settings</a></nav><main><div class="card"><div id="state-line" class="sec-state sec-off"><span class="sec-dot"></span><span id="state-text">Loading…</span></div><div class="sec-help">When a password is set, all destructive actions (relay control, settings changes, OTA, calibration, factory reset) require Basic-Auth credentials. Read-only views (dashboard, status, sensor readings) remain public so the device is still browseable. <strong>Browsers cache the credentials for the session</strong>, so you'll only be prompted on first save after opening the tab.<br><br>The username is empty — leave it blank in the browser prompt and just enter the password.</div><form id="pw-form"><div class="form-group" id="current-row"><label for="current">Current Password</label><input type="password" id="current" autocomplete="current-password"></div><div class="form-group"><label for="new-pw">New Password</label><input type="password" id="new-pw" autocomplete="new-password" minlength="6"></div><div class="form-group"><label for="confirm-pw">Confirm New Password</label><input type="password" id="confirm-pw" autocomplete="new-password" minlength="6"></div><div class="warn">If you forget the password, recovery requires a USB re-flash (which clears NVS). Pick something memorable.</div><div class="save-row"><button type="submit" class="btn-primary" id="save-btn">Set Password</button><span class="msg" id="msg"></span></div></form><div class="danger-row" id="clear-row" style="display:none"><h3 style="font-size:.95rem;margin-bottom:.5rem">Disable authentication</h3><p style="font-size:.82rem;color:#666;margin-bottom:.6rem">Returns the device to open mode — anyone on the LAN can change settings. Only do this if you're behind a tightly-controlled network.</p><button type="button" class="btn btn-danger" id="clear-btn">Clear Password &amp; Open Up</button></div></div></main><script>async function fetchJson(u,o){return(await fetch(u,o)).json()}var enabled=false;function showMsg(t,ok){var m=document.getElementById('msg');m.textContent=t;m.className='msg '+(ok?'ok':'err');m.style.display='inline'}async function loadStatus(){try{var d=await fetchJson('/api/auth/status');enabled=!!d.enabled;var s=document.getElementById('state-line'),t=document.getElementById('state-text'),c=document.getElementById('current-row'),cr=document.getElementById('clear-row'),sb=document.getElementById('save-btn');if(enabled){s.className='sec-state sec-on';t.textContent='Auth enabled — admin password is set';c.style.display='block';cr.style.display='block';sb.textContent='Change Password'}else{s.className='sec-state sec-off';t.textContent='Auth disabled — anyone on the LAN can make changes';c.style.display='none';cr.style.display='none';sb.textContent='Set Password'}}catch(e){showMsg('Could not load status',false)}}document.getElementById('pw-form').addEventListener('submit',async function(e){e.preventDefault();var cur=document.getElementById('current').value,np=document.getElementById('new-pw').value,cp=document.getElementById('confirm-pw').value;if(np.length<6){showMsg('Password must be at least 6 characters',false);return}if(np!==cp){showMsg('Passwords do not match',false);return}if(enabled&&!cur){showMsg('Enter the current password',false);return}var btn=document.getElementById('save-btn'),orig=btn.textContent;btn.disabled=true;btn.textContent='Saving…';try{var r=await fetch('/api/auth/password',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({current:cur,new:np})});var d=await r.json();if(r.ok){showMsg(d.message||'Saved',true);document.getElementById('current').value='';document.getElementById('new-pw').value='';document.getElementById('confirm-pw').value='';setTimeout(loadStatus,500)}else{showMsg(d.error||('HTTP '+r.status),false)}}catch(err){showMsg('Network error',false)}finally{btn.disabled=false;btn.textContent=orig}});document.getElementById('clear-btn').addEventListener('click',async function(){var cur=document.getElementById('current').value;if(!cur){showMsg('Enter the current password to disable',false);return}if(!confirm('Clear the admin password? Anyone on the LAN will be able to change settings.'))return;var btn=this,orig=btn.textContent;btn.disabled=true;btn.textContent='Clearing…';try{var r=await fetch('/api/auth/password',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({current:cur,new:''})});var d=await r.json();if(r.ok){showMsg('Auth disabled',true);document.getElementById('current').value='';setTimeout(loadStatus,500)}else{showMsg(d.error||('HTTP '+r.status),false)}}catch(err){showMsg('Network error',false)}finally{btn.disabled=false;btn.textContent=orig}});loadStatus()</script></body></html>)rawliteral";
}
